...
function StudyView({ skillXp, onTaskToggled, onKnowledgeSummary }: { skillXp: Record<string, number>; onTaskToggled: (branch: string, taskId: string, reason: string, done: boolean, skillAmount: number, completedLevel?: number, generalAmount?: number) => void; onKnowledgeSummary: (completed: number) => void }) {
  ...
  const [openedTask, setOpenedTask] = useState<{ moduleIndex: number; taskIndex: number } | null>(null);
  ...
  return <section className="studyView" aria-label="Panel de estudio">
    ...
    {openedTask && selectedTopic && <div className="studyModalBackdrop" onMouseDown={() => setOpenedTask(null)}><section className="studyModal taskModal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header><div><span>{selectedTopic.modules[openedTask.moduleIndex].name}</span><h2>{selectedTopic.modules[openedTask.moduleIndex].tasks[openedTask.taskIndex].name}</h2></div><button onClick={() => setOpenedTask(null)}>×</button></header><p>{selectedTopic.modules[openedTask.moduleIndex].tasks[openedTask.taskIndex].detail || 'Revisa este ejercicio y aplica lo aprendido.'}</p><button className="saveStudy" onClick={() => { toggleStudyTask(openedTask.moduleIndex, openedTask.taskIndex); setOpenedTask(null); }}>{selectedTopic.modules[openedTask.moduleIndex].tasks[openedTask.taskIndex].done ? 'Marcar como pendiente' : '✓ Marcar como listo'}</button></section></div>}
